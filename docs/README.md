...
How to use it:
  ...
  - Open the console and type or copy-paste the following command:

  EnableStatReviewMenu | > Change Name, Race, Birthsign or Class.

  - Edit anything you want.

  - Close all UI and wait half a second. You'll notice all your stuff is back.

  - You can reopen Stat Review all the times you want, just make sure to unpause the game in between them.

  - Once you're done make a new save, quit the game and launch it again.
...
Some notes:
  ...
  # I STRONGLY suggest you DON'T use the following.
    ...
    They don't provide anything that isn't in EnableStatReviewMenu.
       ...
       D |   EnableNameMenu       | > Change your Name
       O |   EnableRaceMenu       | > Change your Race
       N |   EnableBirthMenu      | > Change your Birthsign
       T |   EnableClassMenu      | > Change your Class
...
| NO DEADLINES, NO PROMISES. | Features are most likely to be implemented in the presented order.
  ...
  - Support any combination of race/birthsign mods
    + Birthsigns are already recognised automatically from your load order
    + Compatibility modules are also recognised automatically, so long as you didn't change their addon file names.
    # Races *will* get automated too. I'm waiting for the necessary API changes to be able to do it.
    # Some mods may still require compatibility patch-modules even after races are automated, especially if they use scripts.
